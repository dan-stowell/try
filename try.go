...
func main() {
	...
	// Extract the base name of the temporary directory, which includes the prefix we gave it
	tempDirBase := filepath.Base(tempDir)

	// The branch name should be the full tempDirBase, which already contains the branchPrefix
	// and the unique suffix generated by os.MkdirTemp.
	branchName := tempDirBase

	// Ensure the branch name is still within 24 characters
	if len(branchName) > 24 {
		branchName = branchName[:24]
	}

	// Remove leading/trailing hyphens that might result from truncation or empty parts
